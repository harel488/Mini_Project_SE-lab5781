...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        assertNull(sphere.findIntersections(new Ray(new Vector(1, 1, 0), new Point3D(-1, 0, 0))),"Ray's line out of sphere");
        ...
        assertEquals( List.of(p2), sphere.findIntersections(new Ray( new Vector(3, 1, 0),new Point3D(0.5, 0.5, 0))),
                "Ray from inside sphere");

        // TC04: Ray starts after the sphere (0 points)
        assertNull(sphere.findIntersections(new Ray( new Vector(3, 1, 0),new Point3D(2, 1, 0))),
                "Sphere behind Ray");
        ...
        assertNull(sphere.findIntersections(new Ray( new Vector(1, 1, 0),new Point3D(2, 0, 0))),
        ...
        assertNull(sphere.findIntersections(new Ray( new Vector(0, 1, 0),new Point3D(1, 1, 0))),
                "Line through O, ray from sphere outside");

        // TC18: Ray starts after sphere (0 points)
        assertNull(sphere.findIntersections(new Ray( new Vector(0, 1, 0),new Point3D(1, 2, 0))),
                "Line through O, ray outside sphere");

        // **** Group: Ray's line is tangent to the sphere (all tests 0 points)
        // TC19: Ray starts before the tangent point
        assertNull(sphere.findIntersections(new Ray( new Vector(1, 0, 0),new Point3D(0, 1, 0))),
                "Tangent line, ray before sphere");

        // TC20: Ray starts at the tangent point
        assertNull(sphere.findIntersections(new Ray( new Vector(1, 0, 0),new Point3D(1, 1, 0))),
                "Tangent line, ray at sphere");

        // TC21: Ray starts after the tangent point
        assertNull(sphere.findIntersections(new Ray( new Vector(1, 0, 0),new Point3D(2, 1, 0))),
                "Tangent line, ray after sphere");

        // **** Group: Special cases
        // TC22: Ray's line is outside, ray is orthogonal to ray start to sphere's
        // center line
        assertNull(sphere.findIntersections(new Ray( new Vector(0, 0, 1),new Point3D(-1, 0, 0))),
